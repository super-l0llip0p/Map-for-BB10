package cc.markc.pureatlas.overlay;
...
public class BusLineOverlay {
    ...
    private MarkerOptions getMarkerOptions(int index) {
        ...
        if (index == 0) {
            ...
            options.icon(getBusBitmapDescriptor(getTitle(index)));
        }
        return options;
    }
    ...
    protected BitmapDescriptor getBusBitmapDescriptor(String title) {
        LayoutInflater inflater = LayoutInflater.from(mContext);
        View v = inflater.inflate(R.layout.item_poi, null);
        TextView poiNameText = v.findViewById(R.id.marker_title);
        poiNameText.setText(title);
        busBit = BitmapDescriptorFactory.fromView(v);
        return busBit;
    }
